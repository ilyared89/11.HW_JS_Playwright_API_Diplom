...
test.describe('UI · Registration @UI @AUTH', () => {
  test('Successful registration with random data @SMOKE', async ({ registerPage }) => {
    ...
    await expect(registerPage.resultMessageLocator).toContainText('Your registration completed');
  ...
  test('Shows error for mismatched passwords', async ({ registerPage }) => {
    ...
    await registerPage.fillForm(user);
    await registerPage.submit();
    await expect(registerPage.validationErrorsLocator.first()).toContainText('The password and confirmation password do not match');
