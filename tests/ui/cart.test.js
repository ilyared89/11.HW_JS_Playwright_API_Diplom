...
test.describe("UI · Cart @UI @CART", () => {
  test("Add product to cart and remove it @SMOKE", async ({
    homePage,
    productPage,
    cartPage,
  }) => {
    ...
    await expect(productPage.notificationLocator).toContainText(
    ...
    await expect(cartPage.emptyCartMessageLocator).toContainText(
